<div id="update_job_history" class="tab-pane fade" style="border-radius:0px;">
    <br>
        <h5 class="table-title" style="border-bottom: 3px solid black !important;">JOB HISTORY</h5>
            <table id="job_history_table_orig" class="table table-bordered table-hover table-striped job_history_table_orig" style="display: none;">
                <thead class="thead-educational">
                    <tr>
                        <th style="border:none;"> COMPANY NAME</th>
                        <th style="border:none;"> JOB DESCRIPTION</th>
                        <th style="border:none;"> JOB POSITION</th>
                        <th style="border:none;"> CONTACT NUMBER</th>
                        <th style="border:none;" colspan="2"> INCLUSIVE YEARS</th>
                    </tr>
                </thead>
                <tbody id="job_history_table_tbody">
                </tbody>
            </table>
</div>